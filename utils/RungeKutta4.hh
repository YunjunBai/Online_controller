 ...
 **/
template<class RHS, class ds_type, class input_type,class F1, class state_type>
void runge_kutta_fixed4(RHS rhs, ds_type &x, input_type &u, F1 &dis, state_type & w2_lb, state_type & w2_ub, const int dim, const double tau, const int nint=10) noexcept {
  ds_type k[4];
  ds_type tmp;
  ...
  for(int t=0; t<nint; t++) {
    ...
    dis.intersection(x,w2_lb,w2_ub);
  }
}
