...
void  main_parameters(const int p1, const int p2){
  ...
  state_type s_lb={-10,-2*M_PI-0.000001};
  /* upper bounds of the hyper rectangle */
  state_type s_ub={10,2*M_PI};
  /* grid node distance diameter */
  state_type s_eta={0.025, 0.3};
  ...
  disturbance_type w2_lb={5-2*p1*s_eta[0], 0-2*p2*s_eta[1]};
  disturbance_type w2_ub={9+2*p1*s_eta[0], M_PI*0.3+2*p1*s_eta[1]};
...
auto rs_repost = [&dis,w2_lb,w2_ub,p2](ds_type &y, input_type &u, bool &neigbour) -> void {
 ...
 // std::cout << "Number of transitions: " << tf_o1d.get_no_transitions() << std::endl;
  ...
  state_type max_dynamic = {{(i_ub[0]+w_1[0])*p2,(i_ub[0]+w_2[1])*p2}};
