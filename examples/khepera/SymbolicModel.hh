  ...
  BDD compute_gb(const Cudd& manager, F1& system_post, F2& radius_post, F3&& avoid, size_t& no_trans) {
    ...
    for(abs_type i=0; i<N; i++) {
      ...
      for(abs_type j=0; j<M; j++) {
        ...
        /* modification to account 2pi for periodicity of the dynamics in the
         * 3rd dimension starts here */ 

        /* determine the cells which intersect with the attainable set: 
         * discrete hyper interval of cell indices 
         * [lb[0]; ub[0]] x [lb[1] ; ub[1]] 
         * covers attainable set */

        /* check out of bounds for dim 1 and 2 */
        ...
        /* process third coordinate */
        double right = x[2]+r[2]+m_z[2];
        double left = x[2]-r[2]-m_z[2];
        /* if left is less than lower left and right is larger than upper right */
        if((left <=( lower_left[2]-eta[2]/2.0 )) && (right >= (upper_right[2]+eta[2]/2.0))) {
          continue;
        }
        if((left >=( lower_left[2]-eta[2]/2.0 )) && (right <= (upper_right[2]+eta[2]/2.0))) {
          lb[2] = static_cast<abs_type>((left-lower_left[2]+eta[2]/2.0)/eta[2]);
          ub[2] = static_cast<abs_type>((right-lower_left[2]+eta[2]/2.0)/eta[2]);
           /* compute BDD of post */
          ...
          /* check if right is larger than M_PI */
          if( (right>M_PI) && ((right-2*M_PI) <= (upper_right[2]+eta[2]/2.0))) {
            lb[2] = static_cast<abs_type>((left-lower_left[2]+eta[2]/2.0)/eta[2]);
            ub[2] = static_cast<abs_type>((M_PI-lower_left[2]+eta[2]/2.0)/eta[2]);
             /* compute BDD of post */
            BDD bdd_k = m_post.interval_to_bdd(manager,lb,ub);
            /* add to transition function */
            tf = tf | (bdd_i & bdd_j & bdd_k);

            lb[2] = static_cast<abs_type>((-M_PI-lower_left[2]+eta[2]/2.0)/eta[2]);
            ub[2] = static_cast<abs_type>(((right-2*M_PI)-lower_left[2]+eta[2]/2.0)/eta[2]);

             /* compute BDD of post */
            bdd_k = m_post.interval_to_bdd(manager,lb,ub);
            /* add to transition function */
            tf = tf | (bdd_i & bdd_j & bdd_k);
             
          }

          /* check if left is smaller than -M_PI */
          if( (left<-M_PI) && ((left+2*M_PI) >= (lower_left[2]-eta[2]/2.0))) {
            lb[2] = static_cast<abs_type>((-M_PI-lower_left[2]+eta[2]/2.0)/eta[2]);
            ub[2] = static_cast<abs_type>((right-lower_left[2]+eta[2]/2.0)/eta[2]);
             /* compute BDD of post */
            BDD bdd_k = m_post.interval_to_bdd(manager,lb,ub);
            ...
            lb[2] = static_cast<abs_type>(((left+2*M_PI)-lower_left[2]+eta[2]/2.0)/eta[2]);
            ub[2] = static_cast<abs_type>((M_PI-lower_left[2]+eta[2]/2.0)/eta[2]);
