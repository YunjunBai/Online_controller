...
public:
  ...
  //#pragma omp threadprivate(tno_pre, tno_post, tcorner_IDs, tout_of_domain)
  /* loop over all cells */
    ...
    // #pragma omp parallel shared(transition_function.m_no_pre, transition_function.m_no_post, transition_function.out_of_domain,transition_function.corner_IDs)
    //   #pragma omp atomic
    //   for (abs_type i = 0; i < N*M; ++i)
    //   {
    //     if (tno_pre[i]!=0)
    //       transition_function.m_no_pre[i]+=tno_pre[i];
    //     if (tno_post[i]!=0)
    //       transition_function.m_no_post[i]+=tno_post[i];
    //     if (tout_of_domain[i])
    //       transition_function.out_of_domain[i]=true;
    //   }
    //   for (abs_type i = 0; i < 2*N*M; ++i)
    //   {
    //     if (tcorner_IDs[i]!=0)
    //     {
    //       transition_function.corner_IDs[i]=tcorner_IDs[i];
    //     }
    //   }
   ...
   /*start big loop untill the recompute_queue become empty*/
    ...
    std::cout<<"total percent" <<coun<<std::endl;
     ...
     for(abs_type i=0; i<N; i++) {
      ...
      /* allocate memory for pre list */
      new_transition.init_transitions(sum);
